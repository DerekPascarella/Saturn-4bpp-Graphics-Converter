...
## Example Usage
Convert a 64x16 4bpp graphic file named `TEX.BIN` to bitmap using the first palette index (i.e., `0`) from palette file `TEX.PAL`.
```
4bpp-2-bmp TEX.BIN TEX.PAL 0 64 16
```
Convert a 64x16 bitmap named `TEX.BIN.BMP` to 4bpp graphic format using the first palette index (i.e., `0`) from palette file `TEX.PAL`.
```
bmp-2-4bpp TEX.BIN.BMP TEX.PAL 0 64 16
```

## Tutorial
[Malenko](https://segaxtreme.net/members/malenko.22808/) published a fantastic tutorial on ripping and replacing SEGA Saturn graphic assets on the [SegaXtreme](https://segaxtreme.net) forums. It also details how to extract palette data. Click the preview screenshot below to access it.
